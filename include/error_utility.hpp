...
struct stream_error_type {
    ...
    friend bool
    operator == (const stream_error_type& lhs, const stream_error_type& rhs)
    {
 ...
 */
constexpr stream_error
make_stream_error(stream_status status, stream_error_message code) noexcept
{
    return stream_error(status, code);
}

constexpr stream_error
make_stream_error(stream_status status) noexcept
{
...
public:
    explicit
    stream_exception(const char * what_arg);
    explicit
    stream_exception(const std::string& what_arg);
    explicit
    stream_exception(const stream_error& err);
    explicit
    stream_exception(const stream_status& status);
    explicit
    stream_exception(const stream_status& status, const stream_error_message& code);
    const stream_error&
    error() const noexcept;
private:
    const std::string
    _make_error_string(const stream_error& err);
 ...
 */
inline stream_error_callback
default_stream_error_callback() noexcept
{
...
#endif // ifndef ZAUDIO_ERROR_UTILITY
