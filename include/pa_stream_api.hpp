...
constexpr PaSampleFormat
_type_to_pa_sample_format() noexcept
{
    return -1;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::f32> >() noexcept
{
    return paFloat32;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::i8> >() noexcept
{
    return paInt8;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::u8> >() noexcept
{
    return paUInt8;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::i16> >() noexcept
{
    return paInt16;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::i24> >() noexcept
{
    return paInt24;
}
...
template <>
constexpr PaSampleFormat
_type_to_pa_sample_format<sample<sample_format::i32> >() noexcept
{
    return paInt32;
}
}
...
public:
    static_assert(
        internal::_type_to_pa_sample_format<sample_t>() != -1,
        "Requested Sample Format Not Supported By PortAudio API");
    pa_stream_api()
    {
    ...
    virtual std::string
    get_error_string(const stream_error& err) noexcept
    {
        return err.second;
    }

    virtual stream_error
    open_stream(const stream_params<sample_t>& params) noexcept
    {
    ...
    virtual stream_error
    is_configuration_supported(const stream_params<sample_t>& params) noexcept
    {
...
private:
    ...
    static int
    _pa_stream_api_callback(const void * input, void * output, unsigned long frameCount,
    ...
    device_info
    _pa_device_info_to_native(const PaDeviceInfo * info)
    {
    ...
    std::tuple<PaStreamParameters, PaStreamParameters, double>
    _native_params_to_pa(const stream_params<sample_t>& params)
    {
        PaStreamParameters inparams;
        PaStreamParameters outparams;
...
#endif // ifndef pa_stream_api_hpp
