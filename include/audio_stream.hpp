...
public:
    ...
    callback
    exchange_callback(callback&& cb);

    stream_error_callback
    exchange_error_callback(stream_error_callback&& cb);

    const stream_params_type&
    params() noexcept;

    void
    params(const stream_params_type& p) noexcept;
...
template <typename sample_t>
stream_error
audio_stream<sample_t>::start() noexcept
{
...
template <typename sample_t>
stream_error
audio_stream<sample_t>::pasue() noexcept
{
...
template <typename sample_t>
stream_error
audio_stream<sample_t>::stop() noexcept
{
...
template <typename sample_t>
stream_error
audio_stream<sample_t>::playback_state() noexcept
{
    return _context.get().api()->playback_state();
}

template <typename sample_t>
typename audio_stream<sample_t>::callback
audio_stream<sample_t>::exchange_callback(callback&& cb)
{
    auto&& out = _callback;
    ...
    if (lk.try_lock_for(std::chrono::seconds(1))) {
        ...
        throw stream_exception(make_stream_error(stream_status::system_error,
                "Unable to aquire lock to swap callback."));
    }
}

template <typename sample_t>
stream_error_callback
audio_stream<sample_t>::exchange_error_callback(stream_error_callback&& cb)
{
    ...
    if (lk.try_lock_for(std::chrono::seconds(1))) {
        ...
        throw stream_exception(make_stream_error(stream_status::system_error,
                "Unable to aquire lock to swap error callback."));
    }
}
...
template <typename sample_t>
void
audio_stream<sample_t>::params(const stream_params_type& p) noexcept
{
    _params = p;
}
...
template <typename sample_t>
double
audio_stream<sample_t>::cpu_load() noexcept
{
...
template <typename sample_t>
void
audio_stream<sample_t>::destroy() noexcept
{
...
stream_error
start_stream(stream&& s) noexcept
{
    return s.start();
}
...
template <typename stream>
stream_error
pause_stream(stream&& s) noexcept
{
    return s.pause();
}
...
audio_stream<typename std::decay<sample_t>::type>
make_audio_stream(args_t&& ... args)
{
    return audio_stream<typename std::decay<sample_t>::type>(args ...);
}
}

#endif // ifndef ZAUDIO_AUDIO_STREAM
