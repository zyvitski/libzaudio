...
{
std::ostream&
operator << (std::ostream& os, sample_format format)
{
    switch (format) {
...
std::string
stream_status_string(stream_status status)
{
    switch (status) {
...
} // stream_status_string

std::ostream&
operator << (std::ostream& os, stream_status status)
{
...
std::ostream&
operator << (std::ostream& os, const stream_error& err)
{
...
stream_exception::stream_exception(const char * what_arg) : std::runtime_error(what_arg), _error(
        stream_status::system_error, what_arg){ }

stream_exception::stream_exception(const std::string& what_arg) : std::runtime_error(what_arg), _error(
        stream_status::system_error, what_arg.c_str()){ }

stream_exception::stream_exception(const stream_error& err) : std::runtime_error(_make_error_string(err)),
    _error(err){ }

stream_exception::stream_exception(const stream_status& status) : std::runtime_error(_make_error_string(
          make_stream_error(status))), _error(status, stream_status_string(status).c_str()){ }

stream_exception::stream_exception(const stream_status& status,
  const stream_error_message                          & code) : std::runtime_error(_make_error_string(make_stream_error(
          status,
          code))),
    _error(status, code){ }

const stream_error&
stream_exception::stream_exception::error() const noexcept
{
    return _error;
}

const std::string
stream_exception::_make_error_string(const stream_error& err)
{
...
std::ostream&
operator << (std::ostream& os, const device_info& info)
{
    ...
    os << "\tDefault Input Latency Range:  <" << info.default_low_input_latency.count() << "s , "
       << info.default_high_input_latency.count() << "s>" << std::endl;
    os << "\tDefault Output Latency Range: <" << info.default_low_ouput_latency.count() << "s , "
       << info.default_high_output_latency.count() << "s>" << std::endl;
    return os;
}

std::ostream&
operator << (std::ostream& os, const std::vector<device_info>& vinfo)
{
